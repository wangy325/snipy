...
/**
 * {@link java.time} 包是jdk8 新增的日期时间包，
 * 可以完全替代之前的日期时间系统，包括：
 *
 * <li>{@link Date}</li>
...
public class JTCommonUsage {
    ...
    /**
     * If the pattern like 'yyyy', result {@link LocalDateTime}
     * could be like 'yyyy-01-01 00:00:00'.<br>
     * If the pattern like 'yyyy-MM', result {@link LocalDateTime}
     * could be like 'yyyy-MM-01 00:00:00'.<br>
     * If the pattern like 'yyyy-MM-dd', result {@link LocalDateTime}
     * could be like 'yyyy-MM-dd 00:00:00'.<br>
     * Other patterns acts the same.
     * <p>
     *
     * <b>Important:</b> the pattern and the the
     * {@link LocalDateTime#parse(CharSequence, DateTimeFormatter)}
     * method's input
     * {@link CharSequence} must match.
     * e.g. The following method call will throw
     * {@link java.time.format.DateTimeParseException}:
     * 
     * <pre>
     * LocalDateTime localDateTime = LocalDateTime
     *         .parse("2021", dtfBuilder("yyyy-MM"));
     * </pre>
     *
    ...
    static Date getStartOfDay(String dateString) throws ParseException {
        ...
        LocalDate localDate = LocalDate
                .parse(dateString, dtfBuilder(yyyy_MM_dd));
        ...
        // sdfBuilder()
        // .parse(zonedDateTime
        // .format(dtfBuilder(yyyy_MM_dd_HH_mm_ss)));
    }
    ...
    static Date getEndOfDay() {
        // 由于dtfBuilder的设置 这里的时间已经是 00:00:00
        LocalDateTime localDateTime = LocalDateTime
                .parse("2020-12-21", dtfBuilder(yyyy_MM_dd));

        // plusXxx()方法没有提供毫秒/微秒的相应方法，直接到纳秒；
        // 相应的，可以使用plus方法指定单位
    ...
    static Date getFirstDayOfYear(String yearString) {
        LocalDateTime localDateTime = LocalDateTime
                .parse(yearString, dtfBuilder(yyyy));
        return Date.from(localDateTime
                .atZone(ZoneId.systemDefault())
                .toInstant());
    }
    ...
    public static void main(String[] args) throws ParseException {
        System.out.println(sdfBuilder(yyyy_MM_dd_HH_mm_ss)
                .format(getStartOfDay()));

        System.out.println(sdfBuilder(yyyy_MM_dd_HH_mm_ss)
                .format(getStartOfDay("2021-02-02")));

        System.out.println(sdfBuilder(yyyy_MM_dd_HH_mm_ss_SSS)
                .format(getStartOfDay(
                        LocalDate
                                .parse("2021-02-22",
                                        dtfBuilder(yyyy_MM_dd)))));
        ...
        System.out.println(sdfBuilder(yyyy_MM_dd_HH_mm_ss_SSS)
                .format(getEndOfDay("2020-12-21")));

        System.out.println(sdfBuilder(yyyy_MM_dd)
                .format(getFirstDayOfMonth("2020-12-18")));

        System.out.println(sdfBuilder(yyyy_MM_dd_HH_mm_ss)
                .format(getFirstDayOfYear("2012")));
    }
}
