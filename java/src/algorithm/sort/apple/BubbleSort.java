 ...
 * 时间复杂度分析：
 * <p>
 * 先抛结论：对于N个互异数，通过交换相邻元素进行排序的
 * 任何算法（冒泡，插入，选择）时间复杂度都为O(N^2)。
 * <p>
 * 对于互异数组（不考虑数组内有相同元素的情况）a[N]，
 * 若<code>a[i] > a[j](i<j)</code>，称（a[i],a[j]）为a[N]的一个逆序对；
 * 反之，若<code>a[i] > a[j](i>j)</code>，称（a[i],a[j]）为a[N]的一个有序对。
 * <br>
 * 事实上，对互异数排序就是将逆序对通过位置交换变为有序对的过程，
 * 因此，有多少逆序对，就需要进行多少次元素交换。
 * <br>
