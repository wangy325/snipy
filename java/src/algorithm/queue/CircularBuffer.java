 ...
 * <p>
 * 另一种实现思路：<a href="https://www.baeldung.com/java-ring-buffer">Implementing a
 * Ring Buffer in Java</a>
 * <p>
 * 上述两种思路的主要差别是：
 * <ul>
 * <li>
 ...
 * </pre>
 * <p>
 * 在之前的顺序队列中，如果<code>write = N && read!=0</code>的情况下，
 * 数组的空间可以重复利用，当时的处理办法是对数组中的元素进行"整理"；
 * <p>
 * 在循环队列中，我们无需再这样做，队列中的slot可以重复使用，
 * slot的值可以一直被覆盖写入，而队列真正
 ...
 *      事实上，由于循环数组的slot一直在重用，故write的取值总介于[0，N)之间，
 *      因此当有元素入队时，可用
 ...
 * </pre>
 * </p>
 * 不难发现，如果read指针追上write指针，即read==write时，队列为空；
 * 反之，若write指针追上read指针，即read==write时，
 * 队列为满。<b>如何消除歧义？</b>
 * 
 * <p>
 * 有2种办法消除歧义：
 ...
 * <li>
 * 2. 取个巧，将write指针对应的slot闲置，如果发现write指针的下一个指针是read指针的时候，
 * 即认为队列满了。这种情况下，队列实际的容量为N-1。
 * </li>
 * </ul>
 * <p>
 * 如果按照方法2，队列为空的判断条件不变，仍为<code>read == write</code>；
 * 但是队列为满的条件变了，即若
 * <code>
 *     read == (write + 1) % N
 * </code>
